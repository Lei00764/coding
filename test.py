...
class IOULoss(nn.Module):
    def forward(self, pred, target, weight=None):
        ...
        pred_aera = (pred_right - pred_left) * (pred_bottom - pred_top)

        w_intersect = torch.min(pred_right, target_right) - torch.max(
            pred_left, target_left
        )
        w_intersect = w_intersect.clamp(min=0)
        h_intersect = torch.min(pred_bottom, target_bottom) - torch.max(
            pred_top, target_top
        )
